...
@section('content')
    ...
    <div class="text-center juices-filter filter-btn" id="mgt9">


        @foreach ($product->categories as $pc)
            {{-- <button class="btnbtn3 filter" data-filter=".category_{{ $pc->id }}">{{ $pc->title }}</button> --}}
            <button onclick="filterItems({{ $pc->title }} )" class="btnbtn3 filterButton"
                data-filter=".category_{{ $pc->id }}">
                {{ $pc->title }}</button>
        @endforeach
    </div>

    <div class="container" id="mgt3" style=" padding-bottom:100px">
        <div class="row text-center" id="mix-wrapper">

            @foreach ($related_products as $p)
                <div class="col-lg-4 col-md-6 col-sm-6 pro-f itemGrid  item filterDiv {{ $p->title }}">
                    <a href="{{ route('eshop_product_path', [$p->id, $seo_url]) }}"> <img id="impic"
